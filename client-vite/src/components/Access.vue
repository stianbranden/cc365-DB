...
async function deleteAccess(){
    const response = await fetch(`${VITE_API_ROOT}access/${access_id}`, {
        method: 'DELETE', 
        headers: {
...
async function getAccess(){
    access.value = await fetch(`${VITE_API_ROOT}access/${access_id}`).then(response=>response.json())
...
function changeAccess(data){
    fetch(`${VITE_API_ROOT}access/${access_id}`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json'
        },
...
function updateUserCount(){
    fetch(`${VITE_API_ROOT}access/${access_id}/users`)
